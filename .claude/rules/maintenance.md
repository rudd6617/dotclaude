...
<!--
讀者：未來 session 的主模型。規定這套制度檔（CLAUDE.md、rules/、skills/、hooks）
怎麼被安全地更新，避免退化成互相矛盾的規則堆。
-->

## 0. 唯一源頭：dotclaude

`CLAUDE.md`、`rules/`、`skills/`、`hooks/`、`settings.json` 是**模板管理檔**，源頭在
...
```
提議修改 rules/judgment.md：
- 現況：<引用現有條文或「無此規則」>
- 觸發事件：<今天發生了什麼，檔案:行號或引文>
- 新條文：<逐字條文>
- 衝突檢查：grep 過 CLAUDE.md + rules/，與 <條目> 不衝突／需同步改 <條目>
改嗎？(y/n)
```
...
| 跨專案的流程/行為教訓（會在別的 repo 再犯） | 先記進專案 `Learning.md`，並**同回合提案**升級進 dotclaude 的 `rules/judgment.md` 或 `CLAUDE.md`（黃區流程） | §2 提案格式 |
...
- **行數上限**：`CLAUDE.md` ≤100 行；`rules/` 每檔 ≤160 行；專案 `Learning.md` ≥40 條或 ≥400 行時跑收斂（合併重複、把重複出現的教訓提案升級成規則、刪過期條目）。超限就精簡，把細節下放引用檔。
