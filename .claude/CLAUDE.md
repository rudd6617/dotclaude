...
IMPORTANT: 行為變更、新功能、架構調整必須經過我確認後才可以執行。提出方案 → 等待確認 → 再動手。
明確錯誤（測試失敗、crash、stack trace）直接修，不需等確認。

1. **理解需求** — 用一句話重述我的需求，確認理解正確
2. **調查** — 讀相關檔案，了解現有架構，use subagents for complex investigation
3. **規劃** — 提出最簡方案。如果有更簡單的做法，選簡單的。涉及架構或多檔案變更時，用 `/plan` 產出摘要。如果執行中出問題，立刻停下重新規劃，不硬撐。
4. **實作** — 寫清晰的代碼。非簡單變更先問自己「數據結構對不對、有沒有不必要的分支」，但不追求抽象的優雅。避免過度設計。
5. **驗證** — 依 `.claude/stack.md` 中定義的指令驗證。確保零破壞性
6. **提交** — 等我確認後再 commit
...
## Task Management
- 多步驟任務用 `tasks/todo.md` 記錄計畫與進度
- 完成一步勾一步，每步附簡要說明
- 結束後在 `tasks/todo.md` 加 review section
