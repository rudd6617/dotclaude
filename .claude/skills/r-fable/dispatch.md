...
**必派 subagent**（符合任一條）：
- 預估讀取 >100 行、或涉及 ≥3 個檔案，只為回答一個問題 → `Explore`
...
**同一方法對同一件事最多嘗試 2 次**（這是總上限；上表各模型的門檻更嚴時從嚴——haiku 錯 1 次就升，不是 2 次）。第 3 次嘗試前必須改變其中之一：模型、方法、或問題本身的拆法。原樣重跑第三次是被禁止的。
